...
| EUA (universitário) | **Callisto** (`callisto.org`) | desde 2015 | Não | precedente operacional do escrow |
...
- **Regime C** porque a base estatutária do desenho (Dodd-Frank §922 da SEC,
  ...
  robusto** — sem risco de anulação judicial.
...
public statements na calibração formal.)
...
<figure markdown>
  ...
  <figcaption>
    Comparação direcional 3 jurisdições, 8 seeds × 24 tiques. <strong>(A)</strong> Trajetória mediana de <code>dano_acumulado</code> com banda interquartílica. <strong>(B)</strong> Dano final com IC bootstrap 95%. A variante EUA (DOJ-ATR: recompensa 15–30% + LCMC ativa) domina; a variante UE (DMA Tool: proteção sem recompensa) fica entre o status quo BR e a variante EUA — consistente com a tese substantiva de que proteção sem incentivo é insuficiente. <strong>Caveat</strong>: capacidade institucional NÃO calibrada (pendência aberta); leituras de volume absoluto não são confiáveis, apenas direcionais.
  </figcaption>
</figure>
...
A pergunta natural depois de enquadrar BR, EUA e UE como variantes paramétricas do mesmo modelo é: **se todas as autoridades antitruste do mundo adotassem LCMC simultaneamente, haveria sinergias?** O modelo examina essa hipótese com duas alavancas independentes e testáveis.
...
1. **Capacidade institucional não calibrada.** `taxa_capacidade` para o DOJ-ATR
   e a DG-COMP não estão calibradas contra orçamento real (pendência aberta
...
3. **Gradiente Saito como proxy.** O perfil de decaimento `saito` em
   ...
   DOJ-ATR estiverem disponíveis.
4. **Callisto não aparece como cenário paramétrico.** Aparece como
   **precedente operacional do escrow**: canal de depósito condicional
...
|---|---|---|
| `"EUA"` | C | Dodd-Frank §922 / DOJ-ATR dão base estatutária robusta à recompensa (sem risco de anulação). Equivalência **bit-a-bit** com `regime="C"` testada em `tests/test_cenarios_v2.py`. |
...
A generalização internacional está **parcialmente fechada**. As ações restantes:
