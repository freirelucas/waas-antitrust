...
| **Capital social organizacional** | Bem coletivo produzido como subproduto de relações de obrigação entre pessoas que se conhecem (Coleman 1990, *Foundations of Social Theory* cap. 12). A massa crítica de cooperação interna que a LCMC mobiliza é capital social no sentido de Coleman, não bem quase-público à Samuelson. |
...
| **Interesse público em detecção e cessação** | Categoria dogmática (Lei 9.784/99 Art. 2º) que sustenta a re-caracterização da recompensa sob Art. 12 sem criar categoria constitutiva nova. |
...
| **Janela de adesão pós-abertura (R29)** | Janela de `janela_adesao_pos_abertura` tiques (default 10) que se abre quando uma firma atinge massa crítica e o escrow é aberto. Durante a janela, trabalhadores da mesma firma que ainda não cooperaram podem aderir à classe dos lenientes e receber desconto progressivo por ordem de chegada (faixas 100/70/50/30/10% — `descontos_faixas_adesao`). Espelha o Art. 86 da Lei 12.529/2011 operado DENTRO da firma aberta. Implementação: `AutoridadeAgent.processar_adesao_pos_abertura`. Cenário canônico: `cascata_adesao_progressiva`. |
| **Faixa de desconto por classe (R29)** | Posição $k$ na fila pós-abertura define o fator `descontos_faixas_adesao[k]` aplicado sobre a recompensa $W$. Faixa 0 = imunidade total (depositantes originais); faixas 1..N = decrescentes. Análogo direto do gradiente Saito (2021) calibrado na Camada 4 de [`mecanismo.md`](mecanismo.md) — mesma lógica empírica, operada agora dentro da firma já aberta. |
| **Escrow consolidado por grupo (R30)** | Sob `usar_escrow_consolidado_grupo=True`, firmas no mesmo grupo econômico (jurisdições diferentes da mesma multinacional) somam depósitos para o gatilho de massa crítica. Quando o gatilho consolidado é atingido, abertura simultânea em TODAS as firmas do grupo — modela MoU bilateral CADE-DOJ-ATR 2019 ou cooperação ICN MoU 2001. Implementação: `WaaSModel._abrir_escrow_consolidado_por_grupo`. |
| **Coordenação internacional (R30)** | Parâmetro `coordenacao_internacional ∈ [0, 1]` que amplifica o sinal Schelling erga omnes (R21/v2.D.1): cada abertura em qualquer firma eleva `p_perc` em TODAS as outras firmas globalmente, modelando o efeito notícia ICN/OECD. Implementação: `WaaSModel._aplicar_coordenacao_internacional`. |
