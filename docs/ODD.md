...
> **Status (atualizado — R01 implementado):** o canal de **dissuasão é endógeno**: cada firma viola enquanto sua atratividade $g_i$ = ganho/sanção supera a detecção percebida $p$, que sobe por expectativa adaptativa quando o canal WaaS opera. Em simulação, os Regimes B/C reduzem as violadoras ativas a zero enquanto o Regime A as faz crescer — sustentando a **direção** da proposição (a prova formal segue como conjectura). Ressalva: o `bem_estar` baseado em detecção (VP − FP − FN) **sub-credita a dissuasão** (menos violação ⇒ menos detecção); use `dano_acumulado` (Σ violadoras ativas) como medida coerente até a reponderação do bem-estar (R05).
