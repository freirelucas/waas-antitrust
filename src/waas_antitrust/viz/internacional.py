...
Implementação completa no caderno `notebooks/WaaS_caderno_v2.ipynb`, seção §11.
...
def gerar_figura():
    ...
    warnings.warn(
        "Implementação de internacional ainda no caderno. Migrar para módulo é tarefa "
        "rastreada em docs/DECISIONS.md.",
        FutureWarning,
        stacklevel=2,
    )
    raise NotImplementedError(
        "Implementação atual está em notebooks/WaaS_caderno_v2.ipynb, seção §11."
    )
