...
21 módulos implementados (T01 fechado em jun/2026 + 2 figuras de
auditoria adicionadas para refletir conteúdo do código no site + R29
em jun/2026): `inversao`, `fase`, `cascata`, `erosao`, `painel_macro`,
...
progressivo), e o suporte cromático `paleta`. Cada submódulo expõe
uma função `gerar_figura(...)` que retorna `(fig, ax)` ou
`(fig, axes)` do matplotlib. Mapa figura → página em
