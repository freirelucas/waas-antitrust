"""§6 — amplificação de variedade ashbiana.

Implementação completa no caderno `notebooks/WaaS_caderno_v2.ipynb`, seção §6.
...
def gerar_figura() -> NoReturn:
    ...
    warnings.warn(
        "Implementação de variedade ainda no caderno. Migrar para módulo é tarefa "
        "rastreada em docs/DECISIONS.md.",
        FutureWarning,
        stacklevel=2,
    )
    raise NotImplementedError(
        "Implementação atual está em notebooks/WaaS_caderno_v2.ipynb, seção §6."
    )
